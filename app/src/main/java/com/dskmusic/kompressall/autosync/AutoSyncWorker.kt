...
class AutoSyncWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        val cfg = JobConfig(
            ...
            audioPreset = Settings.autoSyncVideoPreset,
        ...
        for ((file, item) in items) {
            ...
            val result = try {
                when (item.kind) {
                    MediaKind.VIDEO -> CompressionEngine.processVideo(ctx, item, cfg, destDir, videosDir, backupDir)
                    MediaKind.AUDIO -> CompressionEngine.processAudio(ctx, item, cfg, audioDir, backupDir)
                    MediaKind.IMAGE -> CompressionEngine.processImage(ctx, item, cfg, destDir, backupDir)
                }
            } catch (_: Exception) {
                null
            }
